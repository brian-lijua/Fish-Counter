...
VIDEO_OUPUT_NAME = '{}.avi'.format(uuid.uuid4().hex)
VIDEO_FINAL_PATH = Path(os.path.join(VIDEO_OUTPUT_FOLDER, VIDEO_OUPUT_NAME))

def write_to_video(dq, videoWidth, videoHeight):
    if os.path.isdir(VIDEO_OUTPUT_FOLDER) != True:
        os.mkdir(VIDEO_OUTPUT_FOLDER)

    logger.info('Processing: {}'.format(VIDEO_OUPUT_NAME))
    videoWriter = cv2.VideoWriter(os.path.join(VIDEO_OUTPUT_FOLDER, VIDEO_OUPUT_NAME), cv2.VideoWriter_fourcc(*'DIVX'), 30, (videoWidth, videoHeight))
    while True:
        (total, frame) = dq.get()
        h, w = frame.shape[:2]
        c = int(w / 2)
...
def run(video_path):
    ...
    logger.info('Completed - duration: {}, video: {}'.format(int(end_process_time - start_process_time), VIDEO_FINAL_PATH.absolute()))
    return VIDEO_FINAL_PATH.absolute()

if __name__ == '__main__':
    import argparse
    parser = argparse.ArgumentParser(description='File path')
    parser.add_argument('filepath', help='Enter the video absolute path')
